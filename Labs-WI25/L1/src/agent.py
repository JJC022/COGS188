...
class TrivialVacuumEnvironment:
    ...
    def random_agent(self, agent: Agent) -> str:
        ...
        """
        action = random.choice(["Stay", "Right", "Left", "Suck"])
        return action
    ...
    def reflex_agent(self, agent: Agent) -> str:
        ...
        if self.status[agent.location] == "Dirty": 
            action = "Suck"
        elif self.status[agent.location] == "Clean" and agent.location == loc_A:
            action = "Right"
        elif self.status[agent.location] == "Clean" and agent.location == loc_B:
            action = 'Left'
        else:
            action = "Stay"
    ...
    def model_based_agent(self, agent: AgentMemory) -> str:
        ...
        if self.status[agent.location] == "Dirty":
            agent.visited[agent.location] = "Clean"
            return  "Suck"

        if len(agent.visited) == 2 and all(status == "Clean" for status in agent.visited.values()): 
            return "Stay"

        if self.status[agent.location] == "Clean" and agent.location == loc_A:
            return 'Right'

        if self.status[agent.location] == "Clean" and agent.location == loc_B:
            return 'Left'
