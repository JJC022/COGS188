...
class Tree(Graph):
    ...
    def validate_tree(self) -> bool:
        ...
        root_count = sum(1 for node in self.nodes if len(node.parents) == 0)
        if root_count != 1:
            return False
...
class BinarySearchTree(Tree):
    ...
    def validate_bst(self) -> bool:
        ...
        - A binary search tree must have the property that for each node, all nodes in its left subtree have smaller values
        and all nodes in its right subtree have larger values.
        Specifically:
        ...
        if not self.root or len(self.nodes) == 0:
            return False

        if sum(1 for node in self.nodes if len(node.parents) == 0) != 1:
            return False


        if not Tree.validate_tree(self):
            return False
